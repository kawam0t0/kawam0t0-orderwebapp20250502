        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center">
            ...
            <Button
              variant="outline"
              size="sm"
              className="text-black border-black hover:bg-black hover:text-yellow-400 bg-transparent"
              onClick={() => {
                        ...
                        <div className="relative h-24 w-24 bg-yellow-50 rounded-md flex-shrink-0 border-2 border-yellow-300 flex items-center justify-center">
                          {item.imageUrl && item.imageUrl.trim() !== "" ? (
                            <Image
                              ...
                              sizes="96px"
                              onError={(e) => {
                                console.error(`Error loading image for ${item.itemName}, using fallback icon`)
                                e.currentTarget.style.display = "none"
                                // 型キャストを追加
                                const fallbackIcon = e.currentTarget.parentElement?.querySelector(
                                  ".fallback-icon",
                                ) as HTMLElement
                                if (fallbackIcon) {
