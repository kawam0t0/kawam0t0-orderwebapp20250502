...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // メール送信処理を追加
    console.log("=== 部品発注確認メール送信処理開始 ===")

    try {
      console.log("部品発注確認メールを送信中:", storeInfo.email)

      const emailResponse = await fetch(
        `${req.headers.origin || "https://office-supplies-app.vercel.app"}/api/send-parts-order-email`,
        {
          method: "POST",
      ...
      if (!emailResponse.ok) {
        ...
        console.error("部品発注確認メール送信に失敗しました:", errorText)
      } else {
        const emailResult = await emailResponse.json()
        console.log("部品発注確認メール送信成功:", emailResult)
      }
    } catch (emailError) {
      console.error("部品発注確認メール送信エラー:", emailError)
    }

    console.log("=== 部品発注確認メール送信処理完了 ===")
